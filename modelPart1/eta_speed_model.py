...
class NewsEmbedder(nn.Module):
    """A lightweight news feature embedder.

    The original code left this module unimplemented. For the numerical news
    tensor generated by :func:`get_node_related_news_tensor`, a small MLP is
    sufficient. Given ``news_feat`` of shape ``(B, nB, M, d_in)`` it averages over
    the ``M`` dimension and projects the result to ``d_out``.
    """

    def __init__(self, d_in: int = 16, d_out: int = 128):
        super().__init__()
    ...
    def forward(self, news_feat: torch.Tensor) -> torch.Tensor:
        """Return averaged embeddings for each B node.

        Parameters
        ----------
        news_feat : torch.Tensor
            Shape ``(B, nB, M, d_in)``. ``M`` is the max number of news for each
            node after padding.
