...
class PgETADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            t2 = datetime.now()
            logging.info(
                f"[步骤1:读取航次] voyage_id={voyage_id}, "
        ...
        t4 = datetime.now()
        logging.info(
            f"[步骤2:采样 A/B] voyage_id={voyage_id}, A_idx={A_idx}, "
        ...
        last_node = nodes[-1]
        end_time = last_node['timestamp'] if isinstance(last_node['timestamp'], datetime) \
            else datetime.fromisoformat(str(last_node['timestamp']))
        label = torch.tensor((end_time - now).total_seconds() / 3600.0,
                             dtype=torch.float)
        t6 = datetime.now()
        logging.info(
            f"[步骤3:生成标签] voyage_id={voyage_id}, 剩余小时数={label.item():.3f}, "
        ...
        t8 = datetime.now()
        logging.info(
            f"[步骤4:A序列构造] voyage_id={voyage_id}, T_A={A_raw.size(0)}, "
        ...
        t10 = datetime.now()
        logging.info(
            f"[步骤4.5:计算弧长] voyage_id={voyage_id}, n_B={len(dist_seg)}, "
        ...
        t13 = datetime.now()
        logging.info(
            f"[步骤5:同船历史] voyage_id={voyage_id}, n_ship={len(ship_ids)}, "
        ...
        t15 = datetime.now()
        logging.info(
            f"[步骤6:邻船批量查询] voyage_id={voyage_id}, n_B={len(near_rows_list)}, "
        ...
        t17 = datetime.now()
        logging.info(
            f"[步骤6.2:收集 vid2] voyage_id={voyage_id}, n_vids={len(all_vids)}, "
        ...
        t19 = datetime.now()
        logging.info(
            f"[步骤6.3:批量拉取节点] voyage_id={voyage_id}, "
        ...
        t18c = datetime.now()
        logging.info(
            f"[步骤6.3.1:缓存 raw/stat] voyage_id={voyage_id}, "
        ...
        def _process_near(args):
            ...
            # 从缓存里直接取“整条航次原始轨迹 Tensor”
                ...
                dx, dy = latlon_to_local(
                    B_ref.get("latitude") or 0.0, B_ref.get("longitude") or 0.0,
                    B_ref.get("course")or  0.0,
                    r.get("latitude")or 0.0, r.get("longitude")or 0.0
                )
            ...
            logging.info(
                f"  [邻船·结束] voyage_id={voyage_id}, B_index={b_idx}, "
        ...
        t21 = datetime.now()
        logging.info(
            f"[步骤6.4:邻船处理完毕] voyage_id={voyage_id}, "
        ...
        t18 = datetime.now()
        logging.info(
            f"[步骤7:B6特征] voyage_id={voyage_id}, n_B={len(B6_list)}, "
        ...
        t20 = datetime.now()
        logging.info(
            f"[步骤8:新闻特征] voyage_id={voyage_id}, use_news={self.use_news}, "
        ...
        t21 = datetime.now()
        logging.info(
            f"[__getitem__] 完成打包，voyage_id={voyage_id}, 总耗时={(t21 - t0).total_seconds():.3f}s"
        )
